...
export async function deleteForm(formId: string) {
    try {
        ...
        let existingForm: { id: number; settingsId?: number; settingId?: number } | null = null;

        try {
            existingForm = await txClient.form.findFirst({
                where: {
                    formId,
                    userId: user.id,
                },
        ...
        const linkedSettingId = existingForm.settingsId ?? existingForm.settingId;

        await prisma.$transaction(async (tx) => {
            const transactionClient = tx as any;

            try {
                await transactionClient.formResponses.deleteMany({
                    where: {
                        formId: existingForm.id,
                    },
                });
            } catch {
                await transactionClient.formResponse.deleteMany({
                    where: {
                        formId: existingForm.id,
                    },
                });
            ...
            if (!linkedSettingId) return;

            let linkedFormsCount = 0;
            try {
                linkedFormsCount = await transactionClient.form.count({
                    where: {
                        settingsId: linkedSettingId,
                    },
                });
            } catch {
                linkedFormsCount = await transactionClient.form.count({
                    where: {
                        settingId: linkedSettingId,
                    },
                });
            }

            if (linkedFormsCount === 0) {
                await tx.formSettings.delete({
                    where: {
                        id: linkedSettingId,
                    },
                });
...
export async function submitResponse(formId: string, response: string) {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            try {
                await transactionClient.formResponses.create({
                    data: {
                        formId: form.id,
                ...
                try {
                    await transactionClient.formResponse.create({
                        data: {
                            formId: form.id,
...
export async function fetchAllResponseByFormId(formId: string) {
    try {
        ...
        try {
            ...
            form = await (prisma as any).form.findFirst({
                where: {
                    formId,
                    userId: user.id,
                },
        ...
        if (form && !form.formResponse) {
            form.formResponse = form.formResponses ?? [];
        }
